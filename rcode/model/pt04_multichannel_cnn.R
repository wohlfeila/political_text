...
pos_combine <- pos_tag %>% 
  group_by(doc_id) %>% 
  summarise(pos = paste0(upos, collapse = " ")) %>% 
  ungroup() %>% 
  mutate(doc_id = as.numeric(sub("doc", "", doc_id))) %>% 
  arrange(doc_id)

# calculating baseline for model performance - guess every document is democrat
baseline <- 1 - (sum(train_labels) / length(train_labels))
...
# building out pos input -------------------------------------------
# choosing top words to be considering in tokenization  
top_n_words_pos <- 17

# fitting tokenizer
tokenizer_pos <- text_tokenizer(num_words = top_n_words_pos) %>% 
  fit_text_tokenizer(pos_combine$pos)

# creating sequences for text to use in embeddings
sequences_pos <- texts_to_sequences(tokenizer_pos, pos_combine$pos)

# creating uniform feature set
features_pos <- pad_sequences(sequences_pos,  padding = "post", maxlen = max_len)

# modeling ----------------------------------------------------------
embedding_dim <- 512

# building model architecture 
text_input <- layer_input(shape = c(512))
pos_input <- layer_input(shape = c(512))
...
encoded_channel_two <- pos_input %>% 
  layer_embedding(input_dim = top_n_words_pos, 
                  output_dim = embedding_dim, 
                  input_length = max_len) %>% 
  layer_conv_1d(filters = 256, kernel_size = 3) %>% 
  layer_average_pooling_1d(pool_size = 3) %>% 
...
# fitting the model
history <- model %>% fit(
  list(features_text, features_pos), 
  train_labels,
  epochs = 20,
  batch_size = 32,
